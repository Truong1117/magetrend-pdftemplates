 ...
 * @link     https://www.magetrend.com/magento-2-pdf-invoice-pro
 */
class Page extends \Magetrend\PdfTemplates\Model\Adapter\TcPdf\Element
{
    ...
    public function drawBox()
    {
        ...
        if (isset($attributes['background_color'])) {
            ...
            //$this->setTextColor($attributes['background_color']);


            //$this->pdf->setLineWidth(0);
            //$this->pdf->drawRectangle($x1, $y1, $x2, $y2, $method);
            if ($attributes['background_color'] == 'rgba(0, 0, 0, 0)') {
                //$this->pdf->setAlpha(1);
            }

            $fillColor = $this->getPdfColor($attributes['background_color']);
            $this->pdf->Rect($x1, $y1, $x2-$x1, $y2-$y1, 'DF', ['width' => 0], $fillColor);
        }
